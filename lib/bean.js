...
Bean.prototype._onRead = function(data){
  ...
  var seq = (data[0] & 0x60) >> 5; //TODO check if seq in order
  var rsp = (data[0] & 0x80);
  console.log(rsp);
  ...
  //need to do a lookup somehow with 
  var rspType = data[4] & ~(rsp);

  //ideally some better way to do lookup
  if(data[3] === commands.MSG_ID_CC_ACCEL_READ[0] && rspType === commands.MSG_ID_CC_ACCEL_READ[1])
  {
  ...
  } else if(data[3] === commands.MSG_ID_SERIAL_DATA[0] && data[4] === commands.MSG_ID_SERIAL_DATA[1]){

    this.emit('serial', data.slice(5,5+size-2), valid, seq);

  } else if(data[3] === commands.MSG_ID_CC_TEMP_READ[0] && rspType === commands.MSG_ID_CC_TEMP_READ[1]){

    this.emit('temp', data[5], valid, seq);
